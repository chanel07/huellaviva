module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Notificar solo a usuarios de la misma ciudad usando filtros
    const body = {
    ...
    // Si hay ciudad usa filtro, sino notifica a todos
    if (ciudad) {
      body.filters = [
    ...
    // Si no hay usuarios con ese tag, intentar notificar a todos
    if (data.errors && data.errors.includes('All included players are not subscribed')) {
      ...
      return res.status(200).json({ ok: true, fallback: true, id: d2.id });
    }
